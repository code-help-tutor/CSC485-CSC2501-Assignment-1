...
from q1_parse import PartialParse, get_sentence, minibatch_parse
...
def _test_deps(name, pp, stack_idx, n, ex_deps, left=True):
    ...
    if left:
        deps = pp.get_nleftmost(stack_idx, n=n)
    else:
        deps = pp.get_nrightmost(stack_idx, n=n)
...
def test_leftmost_rightmost():
    """Simple tests for the PartialParse.get_nleftmost and rightmost
...
@main.command()
def q1_c():
    '''Run basic tests for question q1c: complete, parse_step, get_nrightmost, get_nleftmost.'''
    click.echo('='*80 + '\nRunning Q1(c) tests.\n' + '='*80)
